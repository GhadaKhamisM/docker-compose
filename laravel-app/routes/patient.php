...
Route::name('patient.')->middleware(['jwt.guard:patient'])->prefix('v1')->group(function () {
    ...
    Route::get('doctors/{doctor}/reviews', 'ReviewController@index')->name('reviews.show');
    
    Route::middleware(['auth.jwt','auth.check:patient','booking.access'])->group(function () {
        Route::post('doctors/{doctor}/times', 'DoctorController@getDoctorAvailableTimes')->name('doctors.times');
        ...
        Route::post('doctors/{doctor}/reviews', 'ReviewController@store')->middleware('review.check')->name('reviews.store');
    });
});